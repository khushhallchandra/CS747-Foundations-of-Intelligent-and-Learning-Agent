...
STRIKER_RADIUS = 20.6
COIN_RADIUS = 15.01
features = np.zeros((26,26),dtype=np.int)
csvPath = 'datacsv.csv'
pos = random.random()
...
def writeDataToCsv(xdata,reward,ydata1,ydata2,ydata3):
    totalfeatures=[]
    for i in range(xdata.shape[0]):
        for j in range(xdata.shape[1]):
            if(xdata[i][j] == 0):
                totalfeatures.append(1)
                totalfeatures.append(0)
                totalfeatures.append(0)
            elif(xdata[i][j] == 1):
                totalfeatures.append(0)
                totalfeatures.append(1)
                ...
                totalfeatures.append(0)
                totalfeatures.append(1)
    totalfeatures.append(int(ydata1/0.1))
    totalfeatures.append(int((ydata2+45)/5))
    totalfeatures.append(int(ydata3/0.1))
    for i in range(int(reward)):
        writer.writerow(totalfeatures)
...
def agent_1player(state):
    ...
    flag = 1
    global pos,angle,force,features
    # print state
    print "******************"
    try:
    ...
    writeDataToCsv(features,reward,pos,angle,force)
    for i in range(26):
        for j in range(26):
            features[i][j] = 0

    for (x,y) in (white+black):
        features[int(x/30)][int(y/30)] = 1
    if len(red) == 1:
        x = red[0][0]
        y = red[0][1]
        features[int(x/30)][int(y/30)] = 2
    #print(features)
